...
El proxy es el transporte del ecosistema CloserClick. Esta nota documenta cómo se
...
- **Directorio**: `https://closer.click/nodes.json` lista los nodos. El cliente
...
3. Agregarlo a `nodes.json` (commit al repo del catálogo `closerclick`).
...
nivel B la comparte vía el 5º pilar (`@closerclick/closer-click-reputation`),
...
### Otros futuros
- **Home preferido en el vault** (`id.closer.click`): que tu proxio elegido viaje
...
- **Directorios comunitarios**: aceptar múltiples `nodes.json` (no solo el de
  closer.click) para no depender de un seed único.
